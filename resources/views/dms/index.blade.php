                        ...
                        <div class="table-responsive">
                            <table id="dtforall" class="table table-striped">
                                ...
                                @foreach($dms as $dm)
                                    @if($dm->status ==0)
                                        <tr>
                                        <td>@if ($dm->bl_number == 0)
                                            No BL/SO number set
                                            @else
                                            {{ $dm->bl_number }}
                                            @endif
                                        </td>
                                        <td><a href="{{ url('dms/edit/'. $dm->id) }}">{{ ucwords($dm->customer->Name) }}</a></td>
                            ...
                            <table id="dtforall" class="table table-striped">
                                ...
                                @foreach($dms as $dm)
                                    @if($dm->status ==1)
                                        <tr>
                                            <td>{{ $dm->bl_number }}</td>
                                            <td><a href="{{ url('/customer-dsr/'.$dm->customer->DCLink) }}">{{ ucwords($dm->customer->Name) }}</a></td>
                                            <td>{{ ucwords($dm->customer->Contact_Person) }}</td>
                                            <td>{{ $dm->customer->Telephone }}</td>
                                            {{--<td>--}}
                                            {{--                                            @foreach($dm->contracts as $contract)--}}
