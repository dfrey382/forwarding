...
//        dd($bl);
    ...
    {
        $data = $request->all();
        $file_no = $request->job_type.'/'.(count(Project::all())+1).'/'.Carbon::now()->format('y').strtoupper($request->branch);
        $data['code_name']= $file_no;
